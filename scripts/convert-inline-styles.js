...
// Note: the above replacement builds a template string; fix formatting to valid JSX
content = content.replace(/className=\{`\$\{styles\.btnPrimary\} \$\{styles\.(gen\d+)\}`\}/g, (m, g1) => {
  return `className={\`${"${styles.btnPrimary}"} ${"${styles." + g1 + "}"}\`}`;
});
// Simpler: replace pattern of earlier with className={`${styles.btnPrimary} ${styles.genX}`}
content = content.replace(/className=\{`\$\{styles\.btnPrimary\} \$\{styles\.(gen(\\d+))\}`\}/g, 'className={`${styles.btnPrimary} ${styles.$2}`}');
...
for (const r of toReplace) {
  // replace only first occurrence to avoid accidental multi-replace
...
css = css.replace(/\.btnPrimary\s*\{\s*margin-top: 16px;\s*\}/, '');
